#!/usr/bin/env bash
# PrimeStride — fix product pages (OFFLINE, no downloads) + rename AI 掌櫃 -> AI 營運大腦
# Safe to re-run. Usage: drop this file in the repo root, then:  bash fix-pages-and-rename.sh
set -e
cd "$(dirname "$0")"

echo "→ checking visuals already in repo (no downloads needed)"
need="atlas-flow-wide.png lyra-flow-wide.png pulse-flow-wide.png knowledge-flow-wide.png product-atlas-eip.png product-lyraai.png product-pulse.png product-knowledge-ai.png"
missing=0
for f in $need; do
  if [ ! -s "public/visuals/$f" ]; then echo "  MISSING: public/visuals/$f"; missing=1; fi
done
if [ "$missing" = "1" ]; then
  echo "Some images are missing — restore them from git history or ask Kimi to re-share."
  exit 1
fi
echo "  all 8 images present"
...
echo "→ renaming AI 掌櫃 → AI 營運大腦 across the site"
python3 - <<'PY2'
import io, os

# --- special copy fixes first (order matters) ---
special = {
...
for root, _, files in os.walk("src"):
    for fn in files:
        if fn.endswith((".ts", ".tsx", ".css")):
            p = os.path.join(root, fn)
            if "掌櫃" in io.open(p, encoding="utf-8").read():
                left.append(p)
if left:
    print("  WARNING — 掌櫃 still present in:")
    for p in left: print(f"    {p}")
else:
    print("  verified: no 掌櫃 left in src/")
PY2

echo ""
echo "DONE. Next:  npm run build  &&  git add -A && git commit -m 'Custom product pages + rename AI 掌櫃 to AI 營運大腦' && git push"
