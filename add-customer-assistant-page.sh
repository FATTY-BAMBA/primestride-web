#!/usr/bin/env bash
# PrimeStride — AI Customer Assistant custom page + EduSense headline tweak (safe to re-run)
# Usage: drop this file in the repo root, then:  bash add-customer-assistant-page.sh
set -e
cd "$(dirname "$0")"
...
dl "https://www.kimi.com/apiv2-files/sign-obj/kimi-fs%2Ffiles%2Fblob%2F65ab953ba7d9bde26d6a3619efe27eb7dd699b89999a37eeffc5a1bf8d843227?filename=ca-flow-wide.png&sig=JMueNX_mA57mBbn6Wiy_vIgGcG73X3GiDIEcHeRv8mE=&t=o" public/visuals/ca-flow-wide.png
...
echo "→ creating src/app/[locale]/products/ai-customer-assistant/page.tsx"
mkdir -p "src/app/[locale]/products/ai-customer-assistant"
cat > "src/app/[locale]/products/ai-customer-assistant/page.tsx" <<'TSX'
...
echo "→ patching products/[slug]/page.tsx (exclude ai-customer-assistant from dynamic route)"
python3 - <<'PY'
import io, sys
path = "src/app/[locale]/products/[slug]/page.tsx"
s = io.open(path, encoding="utf-8").read()
two = 'products.filter((p) => p.slug !== "edusense-ai" && p.slug !== "ai-customer-assistant").map((p) => ({ locale, slug: p.slug }))'
one = 'products.filter((p) => p.slug !== "edusense-ai").map((p) => ({ locale, slug: p.slug }))'
orig = "products.map((p) => ({ locale, slug: p.slug }))"
if '"edusense-ai" && p.slug !== "ai-customer-assistant"' in s:
    print(f"  {path}: already patched, skipped")
elif one in s:
    io.open(path, "w", encoding="utf-8").write(s.replace(one, two, 1))
    print(f"  {path}: 1 edit(s) (added second exclusion)")
elif orig in s:
    io.open(path, "w", encoding="utf-8").write(s.replace(orig, two, 1))
    print(f"  {path}: 1 edit(s)")
else:
    sys.exit(f"ANCHOR NOT FOUND in {path} — repo may have changed; aborting safely.")
PY

echo "→ tweaking EduSense headline (broader than video-only)"
python3 - <<'PY'
import io, os
path = "src/app/[locale]/products/edusense-ai/page.tsx"
if not os.path.exists(path):
    ...
    hits = 0
    for a, b in [
        ('pipeTitle: "From class video to intelligence.",', 'pipeTitle: "From raw class content to intelligence.",'),
        ('pipeTitle: "從課堂影片，到教學智慧。",', 'pipeTitle: "從原始教材，到教學智慧。",'),
    ]:
        if a in s:
            s = s.replace(a, b, 1); hits += 1
    if hits:
        io.open(path, "w", encoding="utf-8").write(s)
        print(f"  {path}: {hits} headline edit(s)")
    else:
        print(f"  {path}: headline already updated, skipped")
PY
...
if ".ca-stats" not in css:
    css += """

/* ── AI Customer Assistant demos ────────────────────── */
...
"""
    ...
    print("  globals.css: customer-assistant styles appended")
else:
    print("  globals.css: customer-assistant styles already present, skipped")
PY

echo ""
echo "DONE. Next:  npm run build  &&  git add -A && git commit -m 'AI Customer Assistant: custom page with flow visual, LINE handoff + weekly gap demos; EduSense headline tweak' && git push"
